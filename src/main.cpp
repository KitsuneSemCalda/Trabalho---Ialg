...
/*
Funcao Vazia Cadastro que gera os valores para serem cadastrados no registro
Medicamentos
*/
void Cadastro() {
  ...
  *TamVetor = 3;
  ...
  for (int i = 0; i != *TamVetor; i++) {
    cout << "Nome do Medicamento: " << VecMed[i].NomeMedicamento << endl;
    cout << "Descrição do Medicamento: " << VecMed[i].Descricao << endl;
    cout << "Laboratorio em que o medicamento foi criado: "
         << VecMed[i].Laboratorio << endl;
    cout << "Codigo de Identificação do medicamento: "
         << VecMed[i].CodigoIndentificacao << endl;
    cout << "Quantidade Disponivel em estoque: "
         << VecMed[i].QuantidadeDisponivel << endl;
    if (VecMed[i].SituacaoProduto == true) {
      cout << "Situação do Produto: Ativo" << endl;
    } else {
      cout << "Situação do Produto: Inativo" << endl;
    }
    cout << "---------------------------------------------------------------"
         << endl;
  }
